// Package imagepreview provides QMUIImagePreviewView - an image preview component
...
// ImagePreviewView displays images with zoom and pan support
...
// NewImagePreviewView creates a new image preview view
func NewImagePreviewView() *ImagePreviewView {
	ipv := &ImagePreviewView{
...
// NewImagePreviewViewWithImages creates a preview view with images
func NewImagePreviewViewWithImages(images []fyne.Resource) *ImagePreviewView {
	ipv := NewImagePreviewView()
	ipv.Images = images
	return ipv
}

// SetImages sets the images to display
func (ipv *ImagePreviewView) SetImages(images []fyne.Resource) {
	ipv.mu.Lock()
	ipv.Images = images
...
func (ipv *ImagePreviewView) SetCurrentIndex(index int) {
	ipv.mu.Lock()
	if index < 0 {
		index = 0
	}
...
func (ipv *ImagePreviewView) Next() {
	ipv.mu.RLock()
...
func (ipv *ImagePreviewView) Previous() {
	ipv.mu.RLock()
	index := ipv.CurrentIndex - 1
	ipv.mu.RUnlock()
...
func (ipv *ImagePreviewView) Dragged(e *fyne.DragEvent) {
...
func (ipv *ImagePreviewView) DragEnd() {}

// Tapped handles taps (dismiss on tap)
func (ipv *ImagePreviewView) Tapped(_ *fyne.PointEvent) {
...
func (ipv *ImagePreviewView) TappedSecondary(_ *fyne.PointEvent) {}

// CreateRenderer implements fyne.Widget
func (ipv *ImagePreviewView) CreateRenderer() fyne.WidgetRenderer {
	ipv.ExtendBaseWidget(ipv)
...
// ImagePreviewViewController manages full-screen image preview
type ImagePreviewViewController struct {
...
// NewImagePreviewViewController creates a new preview controller
func NewImagePreviewViewController() *ImagePreviewViewController {
	return &ImagePreviewViewController{
		PreviewView: NewImagePreviewView(),
	}
}

// Show shows the image preview as a full-screen overlay
func (ipvc *ImagePreviewViewController) Show(window fyne.Window) {
	ipvc.window = window
...
func (ipvc *ImagePreviewViewController) Hide() {
	if ipvc.popup != nil {
		ipvc.popup.Hide()
		ipvc.popup = nil
	}
...
func ShowImages(window fyne.Window, images []fyne.Resource, startIndex int) *ImagePreviewViewController {
	controller := NewImagePreviewViewController()
