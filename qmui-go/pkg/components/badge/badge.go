// Package badge provides badge/notification indicators
package badge
...
// Badge is a badge label component
type Badge struct {
	widget.BaseWidget
...
// NewBadge creates a new badge label
...
func (bl *Badge) SetText(text string) {
	bl.mu.Lock()
	bl.Text = text
	bl.mu.Unlock()
	bl.Refresh()
...
func (bl *Badge) CreateRenderer() fyne.WidgetRenderer {
	bl.ExtendBaseWidget(bl)
