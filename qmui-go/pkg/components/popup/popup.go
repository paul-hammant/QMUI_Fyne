// Package popup provides QMUIPopupMenuView - a customizable popup menu
...
// PopupContainerView is a generic popup container with arrow
type PopupContainerView struct {
	widget.BaseWidget
...
// NewPopupContainerView creates a new popup container
func NewPopupContainerView() *PopupContainerView {
	config := core.SharedConfiguration()
	pcv := &PopupContainerView{
...
func (pcv *PopupContainerView) ShowAt(window fyne.Window, position fyne.Position) {
	pcv.mu.Lock()
	pcv.window = window
	pcv.visible = true
	pcv.mu.Unlock()
...
func (pcv *PopupContainerView) ShowBelowView(window fyne.Window, view fyne.CanvasObject) {
...
func (pcv *PopupContainerView) ShowAboveView(window fyne.Window, view fyne.CanvasObject) {
	viewPos := view.Position()
...
func (pcv *PopupContainerView) Hide() {
	pcv.mu.Lock()
	defer pcv.mu.Unlock()
...
func (pcv *PopupContainerView) IsVisible() bool {
	pcv.mu.RLock()
	defer pcv.mu.RUnlock()
	return pcv.visible
}

func (pcv *PopupContainerView) buildContent() fyne.CanvasObject {
...
func (pcv *PopupContainerView) CreateRenderer() fyne.WidgetRenderer {
	pcv.ExtendBaseWidget(pcv)
	return &popupContainerRenderer{container: pcv}
}

type popupContainerRenderer struct {
	container *PopupContainerView
}
...
// PopupMenuView is a popup menu with multiple items
...
// NewPopupMenuView creates a new popup menu
func NewPopupMenuView() *PopupMenuView {
	config := core.SharedConfiguration()
	pmv := &PopupMenuView{
		PopupContainerView:          NewPopupContainerView(),
...
// NewPopupMenuViewWithItems creates a popup menu with items
func NewPopupMenuViewWithItems(items []*MenuItem) *PopupMenuView {
	pmv := NewPopupMenuView()
	pmv.Items = items
...
func (pmv *PopupMenuView) AddItem(item *MenuItem) {
	pmv.Items = append(pmv.Items, item)
	pmv.buildMenuContent()
}

// RemoveItem removes an item from the menu
func (pmv *PopupMenuView) RemoveItem(index int) {
...
func (pmv *PopupMenuView) buildMenuContent() {
...
func (pmv *PopupMenuView) createItemView(index int, item *MenuItem) fyne.CanvasObject {
...
func (pmv *PopupMenuView) Show(window fyne.Window, position fyne.Position) {
	pmv.buildMenuContent()
	pmv.PopupContainerView.ShowAt(window, position)
}
...
func ContextMenu(window fyne.Window, position fyne.Position, items []*MenuItem) *PopupMenuView {
	menu := NewPopupMenuViewWithItems(items)
