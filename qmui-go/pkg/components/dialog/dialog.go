// Package dialog provides QMUIDialogViewController - custom content dialogs
...
// DialogViewController manages a custom content dialog
type DialogViewController struct {
	// Content
	Title           string
	Message         string
...
// NewDialogViewController creates a new dialog controller
func NewDialogViewController() *DialogViewController {
...
// NewDialogViewControllerWithTitle creates a dialog with a title
func NewDialogViewControllerWithTitle(title string) *DialogViewController {
	dvc := NewDialogViewController()
	dvc.Title = title
	return dvc
}

// NewDialogViewControllerWithContent creates a dialog with custom content
func NewDialogViewControllerWithContent(content fyne.CanvasObject) *DialogViewController {
	dvc := NewDialogViewController()
...
func (dvc *DialogViewController) AddAction(action *DialogAction) {
	dvc.mu.Lock()
...
func (dvc *DialogViewController) AddCancelAction(title string, handler func()) {
	dvc.AddAction(NewCancelAction(title, handler))
}

// AddSubmitAction adds a standard submit action
func (dvc *DialogViewController) AddSubmitAction(title string, handler func()) {
	action := &DialogAction{
		Title:     title,
...
func (dvc *DialogViewController) SetContentView(view fyne.CanvasObject) {
	dvc.mu.Lock()
...
func (dvc *DialogViewController) Show(window fyne.Window) {
	dvc.mu.Lock()
	if dvc.visible {
		dvc.mu.Unlock()
		return
...
func (dvc *DialogViewController) Dismiss() {
	dvc.mu.Lock()
	if !dvc.visible {
		dvc.mu.Unlock()
		return
...
func (dvc *DialogViewController) animateShow() {
...
func (dvc *DialogViewController) animateHide(onComplete func()) {
...
func (dvc *DialogViewController) buildDialogContent() fyne.CanvasObject {
	config := core.SharedConfiguration()
...
func (dvc *DialogViewController) buildHeader() fyne.CanvasObject {
...
func (dvc *DialogViewController) buildButtons() fyne.CanvasObject {
	config := core.SharedConfiguration()
...
func (dvc *DialogViewController) createButton(action *DialogAction) fyne.CanvasObject {
	config := core.SharedConfiguration()
...
func ShowDialog(window fyne.Window, title, message string, actions ...*DialogAction) *DialogViewController {
	dvc := NewDialogViewController()
	dvc.Title = title
	dvc.Message = message
...
func ShowContentDialog(window fyne.Window, title string, content fyne.CanvasObject, actions ...*DialogAction) *DialogViewController {
	dvc := NewDialogViewController()
	dvc.Title = title
	dvc.ContentView = content
	for _, action := range actions {
...
func ShowConfirmDialog(window fyne.Window, title, message string, onConfirm, onCancel func()) *DialogViewController {
...
func ShowInputDialog(window fyne.Window, title, placeholder string, onSubmit func(text string)) *DialogViewController {
