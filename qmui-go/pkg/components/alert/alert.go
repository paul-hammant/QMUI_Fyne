// Package alert provides QMUIAlertController - a customizable alert and action sheet dialog
...
type Action struct {
	...
	Style   ActionStyle
	Enabled bool
	Handler func(controller *AlertController, action *Action)
...
func NewAction(title string, style ActionStyle, handler func(controller *AlertController, action *Action)) *Action {
	return &Action{
		Title:   title,
		Style:   style,
		Enabled: true,
...
// AlertControllerDelegate provides callbacks for alert controller events
type AlertControllerDelegate interface {
	WillShowAlertController(controller *AlertController)
	DidShowAlertController(controller *AlertController)
	WillHideAlertController(controller *AlertController)
	DidHideAlertController(controller *AlertController)
	ShouldHideAlertController(controller *AlertController) bool
}

// AlertController is a customizable alert/action sheet dialog
...
func NewAlertController(title, message string, style ControllerStyle) *AlertController {
...
func (ac *AlertController) AddAction(action *Action) {
	ac.mu.Lock()
...
func (ac *AlertController) AddCancelAction() {
	ac.AddAction(NewAction("Cancel", ActionStyleCancel, nil))
}

// AddTextField adds a text field (alert style only)
func (ac *AlertController) AddTextField(configHandler func(entry *widget.Entry)) {
...
func (ac *AlertController) AddCustomView(view fyne.CanvasObject) {
	ac.mu.Lock()
	ac.customView = view
	ac.mu.Unlock()
}
...
func (ac *AlertController) GetActions() []*Action {
	ac.mu.RLock()
	defer ac.mu.RUnlock()
	return ac.actions
}
...
func (ac *AlertController) GetTextFields() []*widget.Entry {
	ac.mu.RLock()
	defer ac.mu.RUnlock()
...
func (ac *AlertController) ShowIn(window fyne.Window) {
	ac.mu.Lock()
	ac.window = window
	ac.visible = true
	ac.mu.Unlock()
	...
	if ac.Delegate != nil {
		ac.Delegate.WillShowAlertController(ac)
	}
	...
	if ac.Delegate != nil {
		ac.Delegate.DidShowAlertController(ac)
	}
}

// ShowWithAnimated displays the alert with optional animation
func (ac *AlertController) ShowWithAnimated(window fyne.Window, animated bool) {
	ac.ShowIn(window)
}

// Hide hides the alert controller
func (ac *AlertController) Hide() {
	ac.mu.Lock()
	visible := ac.visible
	ac.mu.Unlock()
	...
	if ac.Delegate != nil {
		if !ac.Delegate.ShouldHideAlertController(ac) {
			return
		}
		ac.Delegate.WillHideAlertController(ac)
	}
	...
	if ac.Delegate != nil {
		ac.Delegate.DidHideAlertController(ac)
	}
}

// HideWithAnimated hides the alert with optional animation
func (ac *AlertController) HideWithAnimated(animated bool) {
	ac.Hide()
}

func (ac *AlertController) buildContent() fyne.CanvasObject {
...
func (ac *AlertController) buildAlertContent() fyne.CanvasObject {
...
func (ac *AlertController) buildActionSheetContent() fyne.CanvasObject {
	// Header (title + message)
	var headerObjects []fyne.CanvasObject
...
func (ac *AlertController) createActionButton(action *Action) fyne.CanvasObject {
...
func (ac *AlertController) CreateRenderer() fyne.WidgetRenderer {
	ac.ExtendBaseWidget(ac)
...
// IsAnyAlertControllerVisible returns whether any alert controller is visible
var activeAlerts sync.Map

func IsAnyAlertControllerVisible() bool {
	visible := false
...
func ShowAlert(window fyne.Window, title, message string, actions ...*Action) *AlertController {
	ac := NewAlertController(title, message, ControllerStyleAlert)
	for _, action := range actions {
		ac.AddAction(action)
	}
	if len(actions) == 0 {
...
func ShowConfirm(window fyne.Window, title, message string, onConfirm func(), onCancel func()) *AlertController {
	ac := NewAlertController(title, message, ControllerStyleAlert)
	ac.AddAction(NewAction("Cancel", ActionStyleCancel, func(_ *AlertController, _ *Action) {
		if onCancel != nil {
			onCancel()
		}
	}))
	ac.AddAction(NewAction("OK", ActionStyleDefault, func(_ *AlertController, _ *Action) {
		if onConfirm != nil {
			onConfirm()
		}
	}))
...
func ShowActionSheet(window fyne.Window, title, message string, actions ...*Action) *AlertController {
	ac := NewAlertController(title, message, ControllerStyleActionSheet)
	for _, action := range actions {
		ac.AddAction(action)
	}
	ac.AddCancelAction()
...
func ShowDestructiveConfirm(window fyne.Window, title, message, destructiveTitle string, onDestructive func(), onCancel func()) *AlertController {
	ac := NewAlertController(title, message, ControllerStyleAlert)
	ac.AddAction(NewAction("Cancel", ActionStyleCancel, func(_ *AlertController, _ *Action) {
		if onCancel != nil {
			onCancel()
	...
	ac.AddAction(NewAction(destructiveTitle, ActionStyleDestructive, func(_ *AlertController, _ *Action) {
...
func ShowTextInput(window fyne.Window, title, message, placeholder string, onSubmit func(text string), onCancel func()) *AlertController {
	ac := NewAlertController(title, message, ControllerStyleAlert)
