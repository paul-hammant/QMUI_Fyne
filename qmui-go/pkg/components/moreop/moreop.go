// Package moreop provides QMUIMoreOperationController - grid-style bottom sheet
...
// MoreOperationItem represents an item in the operation sheet
type MoreOperationItem struct {
	Identifier  string
	Title       string
	Icon        fyne.Resource
	Handler     func(item *MoreOperationItem)
	Tag         int
	IsEnabled   bool
	ShowsBadge  bool
	BadgeValue  string
...
// NewMoreOperationItem creates a new operation item
func NewMoreOperationItem(identifier, title string, icon fyne.Resource, handler func(*MoreOperationItem)) *MoreOperationItem {
	return &MoreOperationItem{
...
// MoreOperationItemGroup represents a group/row of items
type MoreOperationItemGroup struct {
	Items []*MoreOperationItem
}

// NewMoreOperationItemGroup creates a new item group
func NewMoreOperationItemGroup(items ...*MoreOperationItem) *MoreOperationItemGroup {
	return &MoreOperationItemGroup{Items: items}
}

// MoreOperationController manages a grid-style bottom sheet
type MoreOperationController struct {
	// Content
	ItemGroups     []*MoreOperationItemGroup
	CancelButton   *MoreOperationItem
	...
	OnShow     func()
	OnDismiss  func()
	OnItemSelected func(item *MoreOperationItem)
...
// NewMoreOperationController creates a new operation controller
func NewMoreOperationController() *MoreOperationController {
	config := core.SharedConfiguration()
	moc := &MoreOperationController{
		ItemGroups:            make([]*MoreOperationItemGroup, 0),
		ItemsPerRow:           4,
		ItemHeight:            80,
...
func (moc *MoreOperationController) AddItemGroup(group *MoreOperationItemGroup) {
	moc.mu.Lock()
...
func (moc *MoreOperationController) AddItems(items ...*MoreOperationItem) {
	moc.AddItemGroup(NewMoreOperationItemGroup(items...))
}

// SetCancelButton sets the cancel button
func (moc *MoreOperationController) SetCancelButton(title string, handler func(*MoreOperationItem)) {
	moc.mu.Lock()
	moc.CancelButton = &MoreOperationItem{
		Identifier: "cancel",
		Title:      title,
		Handler:    handler,
		IsEnabled:  true,
...
func (moc *MoreOperationController) Show(window fyne.Window) {
	moc.mu.Lock()
	if moc.visible {
		moc.mu.Unlock()
		return
...
func (moc *MoreOperationController) Dismiss() {
	moc.mu.Lock()
	if !moc.visible {
		moc.mu.Unlock()
		return
...
func (moc *MoreOperationController) IsVisible() bool {
	moc.mu.RLock()
	defer moc.mu.RUnlock()
	return moc.visible
}

func (moc *MoreOperationController) animateShow(content fyne.CanvasObject) {
	// Slide up animation
	if moc.window == nil {
		return
	}
...
func (moc *MoreOperationController) animateHide(onComplete func()) {
	if onComplete != nil {
		go func() {
...
func (moc *MoreOperationController) buildContent() fyne.CanvasObject {
	moc.mu.RLock()
...
func (moc *MoreOperationController) buildItemGroup(group *MoreOperationItemGroup) fyne.CanvasObject {
...
func (moc *MoreOperationController) buildItemRow(items []*MoreOperationItem) fyne.CanvasObject {
...
func (moc *MoreOperationController) buildItem(item *MoreOperationItem) fyne.CanvasObject {
...
func (moc *MoreOperationController) buildCancelButton(item *MoreOperationItem) fyne.CanvasObject {
...
func (moc *MoreOperationController) selectItem(item *MoreOperationItem) {
	if item.Handler != nil {
		item.Handler(item)
	}
...
type operationItemWidget struct {
	widget.BaseWidget
	controller *MoreOperationController
	item       *MoreOperationItem
	hovered    bool
	mu         sync.RWMutex
}
...
func ShowOperationSheet(window fyne.Window, items []*MoreOperationItem, cancelTitle string) *MoreOperationController {
	moc := NewMoreOperationController()
	moc.AddItems(items...)
	if cancelTitle != "" {
...
func ShowShareSheet(window fyne.Window, onItemSelected func(item *MoreOperationItem)) *MoreOperationController {
	moc := NewMoreOperationController()

	// Example share items (in real use, you'd pass actual icons)
	shareItems := []*MoreOperationItem{
		NewMoreOperationItem("message", "Message", nil, nil),
		NewMoreOperationItem("mail", "Mail", nil, nil),
		NewMoreOperationItem("copy", "Copy Link", nil, nil),
		NewMoreOperationItem("more", "More", nil, nil),
	}
