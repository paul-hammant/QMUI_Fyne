// Package emotion provides an emoji/emoticon picker
package emotion
...
// EmojiPicker displays an emotion picker
...
// NewEmojiPicker creates a new emotion view
func NewEmojiPicker() *EmojiPicker {
	ev := &EmojiPicker{
...
// NewEmojiPickerWithGroups creates an emotion view with emotion groups
func NewEmojiPickerWithGroups(groups []*EmotionGroup) *EmojiPicker {
	ev := NewEmojiPicker()
	ev.Groups = groups
	return ev
}
...
func (ev *EmojiPicker) AddGroup(group *EmotionGroup) {
	ev.mu.Lock()
...
func (ev *EmojiPicker) SetCurrentGroup(index int) {
	ev.mu.Lock()
...
func (ev *EmojiPicker) GetEmotionsForPage(page int) []*Emotion {
	ev.mu.RLock()
	defer ev.mu.RUnlock()
...
func (ev *EmojiPicker) PreviousPage() {
	ev.mu.Lock()
...
func (ev *EmojiPicker) SelectEmotion(emotion *Emotion) {
...
func (ev *EmojiPicker) CreateRenderer() fyne.WidgetRenderer {
	ev.ExtendBaseWidget(ev)
...
func newEmotionButton(emotion *Emotion, view *EmojiPicker) *emotionButton {
	btn := &emotionButton{
		Emotion: emotion,
		View:    view,
	}
...
func NewEmotionInputController(textField *widget.Entry) *EmotionInputController {
	...
	eic.EmojiPicker.OnEmotionSelected = func(emotion *Emotion) {
		eic.InsertEmotion(emotion)
	}

	eic.EmojiPicker.OnDeletePressed = func() {
		eic.DeleteLastCharacter()
	}

	eic.EmojiPicker.OnSendPressed = func() {
		if eic.OnSend != nil {
...
func (eic *EmotionInputController) Show(window fyne.Window) {
	...
	content := container.NewBorder(nil, toolbar, nil, nil, eic.EmojiPicker)

	eic.popup = widget.NewPopUp(content, window.Canvas())

	// Position at bottom of screen
	canvasSize := window.Canvas().Size()
	popupHeight := eic.EmojiPicker.MinSize().Height + 50
