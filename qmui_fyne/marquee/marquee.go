// Package marquee provides a scrolling/marquee text label
package marquee
...
// Marquee is a scrolling text label
type Marquee struct {
	widget.BaseWidget
...
// NewMarquee creates a new marquee label
func NewMarquee(text string) *Marquee {
	ml := &Marquee{
		Text:               text,
		TextStyle:          fyne.TextStyle{},
		TextSize:           14,
...
func (ml *Marquee) SetText(text string) {
	ml.mu.Lock()
	ml.Text = text
	ml.offset = 0
	ml.mu.Unlock()
...
func (ml *Marquee) StartAnimation() {
	ml.mu.Lock()
	if ml.animating {
		ml.mu.Unlock()
		return
...
func (ml *Marquee) CreateRenderer() fyne.WidgetRenderer {
	ml.ExtendBaseWidget(ml)
